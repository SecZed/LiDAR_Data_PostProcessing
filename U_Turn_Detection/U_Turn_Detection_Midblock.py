...
def U_Turn_Lane_To_Judgement_Midblock_EW(lane_from, lane_wb_leave, lane_eb_leave):
    if 'EB' in lane_from:
...
wildcard ='map2023*'
site_list = ['East_Flamingo_Pointe','Eucalyptus','Escondido','Bruce','Clark_Access',
             'Euclid','Mojave','Hildebrand','Channel_10','Grand_Plaza']
...
for site_name in tqdm(site_list):
    ...
    list_vehZone = [vehLane_EB,vehLane_WB]
        ...
        for map_30min in tqdm(dataset):
            for each_direction in list_vehZone:
                df_to_append = extract_vehicle_data(map_30min, each_direction ,
                                                    U_Turn_Lane_To_Judgement_Midblock_EW(each_direction,
                                                                                         vehLane_WB_leave,
                                                                                         vehLane_EB_leave),
                                                    site_name)
